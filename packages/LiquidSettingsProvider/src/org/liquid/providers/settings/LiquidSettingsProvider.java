...
package org.liquid.providers.settings;
...
import org.liquid.provider.LiquidSettings;
...
/**
 * The LiquidSettingsProvider serves as a {@link ContentProvider} for Liquid specific settings
 */
public class LiquidSettingsProvider extends ContentProvider {
    public static final String TAG = "LiquidSettingsProvider";
    ...
    protected final SparseArray<LiquidDatabaseHelper> mDbHelpers
            = new SparseArray<LiquidDatabaseHelper>();
    ...
    static {
        sUriMatcher.addURI(LiquidSettings.AUTHORITY, LiquidDatabaseHelper.TABLE_SYSTEM,
                SYSTEM);
        sUriMatcher.addURI(LiquidSettings.AUTHORITY, LiquidDatabaseHelper.TABLE_SECURE,
                SECURE);
        sUriMatcher.addURI(LiquidSettings.AUTHORITY, LiquidDatabaseHelper.TABLE_GLOBAL,
                GLOBAL);
        sUriMatcher.addURI(LiquidSettings.AUTHORITY, LiquidDatabaseHelper.TABLE_SYSTEM +
    ...
    public boolean onCreate() {
        if (LOCAL_LOGV) Log.d(TAG, "Creating LiquidSettingsProvider");
        ...
        mUriBuilder.authority(LiquidSettings.AUTHORITY);
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (args != null) {
            int reqUser = args.getInt(LiquidSettings.CALL_METHOD_USER_KEY, callingUserId);
        ...
        // Get methods
        if (LiquidSettings.CALL_METHOD_GET_SYSTEM.equals(method)) {
            return lookupSingleValue(callingUserId, LiquidSettings.System.CONTENT_URI, request);
        }
        else if (LiquidSettings.CALL_METHOD_GET_SECURE.equals(method)) {
            return lookupSingleValue(callingUserId, LiquidSettings.Secure.CONTENT_URI, request);
        }
        else if (LiquidSettings.CALL_METHOD_GET_GLOBAL.equals(method)) {
            return lookupSingleValue(callingUserId, LiquidSettings.Global.CONTENT_URI, request);
        }
        ...
        // to do it here.
        if (LiquidSettings.CALL_METHOD_PUT_SYSTEM.equals(method)) {
            enforceWritePermission(org.liquid.framework.Manifest.permission.WRITE_SETTINGS);
        } else {
            enforceWritePermission(org.liquid.framework.Manifest.permission.WRITE_SECURE_SETTINGS);
        }
        ...
        if (LiquidSettings.CALL_METHOD_PUT_SYSTEM.equals(method)) {
            insertForUser(callingUserId, LiquidSettings.System.CONTENT_URI, values);
        }
        else if (LiquidSettings.CALL_METHOD_PUT_SECURE.equals(method)) {
            insertForUser(callingUserId, LiquidSettings.Secure.CONTENT_URI, values);
        }
        else if (LiquidSettings.CALL_METHOD_PUT_GLOBAL.equals(method)) {
            insertForUser(callingUserId, LiquidSettings.Global.CONTENT_URI, values);
        }
    ...
    private Cursor queryForUser(int userId, Uri uri, String[] projection, String selection,
        ...
        LiquidDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName, userId));
    ...
    private Uri insertForUser(int userId, Uri uri, ContentValues values) {
        ...
        if (LiquidDatabaseHelper.TABLE_SYSTEM.equals(tableName)) {
            validateSystemSettingNameValue(name, value);
        } else if (LiquidDatabaseHelper.TABLE_SECURE.equals(tableName)) {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        if (!TextUtils.isEmpty(selection) && selectionArgs.length > 0) {
            ...
            int callingUserId = UserHandle.getCallingUserId();
            LiquidDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName,
                    callingUserId));
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // NOTE: update() is never called by the front-end LiquidSettings API, and updates that
    ...
    /**
     * Tries to get a {@link LiquidDatabaseHelper} for the specified user and if it does not exist, a
     * new instance of {@link LiquidDatabaseHelper} is created for the specified user and returned.
     * @param callingUser
     * @return
     */
    private LiquidDatabaseHelper getOrEstablishDatabase(int callingUser) {
    ...
    /**
     * Check if a {@link LiquidDatabaseHelper} exists for a user and if it doesn't, a new helper is
    ...
    private void establishDbTracking(int userId) {
        ...
        synchronized (this) {
            ...
            if (LOCAL_LOGV) {
                Log.i(TAG, "Checking liquid settings db helper for user " + userId);
            }
            if (dbHelper == null) {
                if (LOCAL_LOGV) {
                    Log.i(TAG, "Installing new liquid settings db helper for user " + userId);
                }
                dbHelper = new LiquidDatabaseHelper(getContext(), userId);
    ...
    private void checkWritePermissions(String tableName) {
        if ((LiquidDatabaseHelper.TABLE_SECURE.equals(tableName) ||
                LiquidDatabaseHelper.TABLE_GLOBAL.equals(tableName)) &&
                getContext().checkCallingOrSelfPermission(
                        org.liquid.framework.Manifest.permission.WRITE_SECURE_SETTINGS) !=
                        PackageManager.PERMISSION_GRANTED) {
            throw new SecurityException(
                    String.format("Permission denial: writing to liquid secure settings requires %1$s",
                            org.liquid.framework.Manifest.permission.WRITE_SECURE_SETTINGS));
        }
    }
    ...
    private String getTableNameFromUriMatchCode(int code) {
        switch (code) {
            case SYSTEM:
            case SYSTEM_ITEM_NAME:
                return LiquidDatabaseHelper.TABLE_SYSTEM;
            case SECURE:
            case SECURE_ITEM_NAME:
                return LiquidDatabaseHelper.TABLE_SECURE;
            case GLOBAL:
            case GLOBAL_ITEM_NAME:
                return LiquidDatabaseHelper.TABLE_GLOBAL;
            default:
                throw new IllegalArgumentException("Invalid uri match code: " + code);
        }
    }
    ...
    private int getUserIdForTable(String tableName, int userId) {
        return LiquidDatabaseHelper.TABLE_GLOBAL.equals(tableName) ?
                UserHandle.USER_OWNER : userId;
    }

    /**
     * Modify setting version for an updated table before notifying of change. The
     * {@link LiquidSettings} class uses these to provide client-side caches.
     * @param uri to send notifications for
     * @param userId
     */
    private void notifyChange(Uri uri, String tableName, int userId) {
        String property = null;
        final boolean isGlobal = tableName.equals(LiquidDatabaseHelper.TABLE_GLOBAL);
        if (tableName.equals(LiquidDatabaseHelper.TABLE_SYSTEM)) {
            property = LiquidSettings.System.SYS_PROP_SLIM_SETTING_VERSION;
        } else if (tableName.equals(LiquidDatabaseHelper.TABLE_SECURE)) {
            property = LiquidSettings.Secure.SYS_PROP_SLIM_SETTING_VERSION;
        } else if (isGlobal) {
            property = LiquidSettings.Global.SYS_PROP_SLIM_SETTING_VERSION;
        }
    ...
    private void validateSystemSettingNameValue(String name, String value) {
        LiquidSettings.Validator validator = LiquidSettings.System.VALIDATORS.get(name);
        if (validator == null) {
    ...
    private void validateSecureSettingValue(String name, String value) {
        LiquidSettings.Validator validator = LiquidSettings.Secure.VALIDATORS.get(name);
