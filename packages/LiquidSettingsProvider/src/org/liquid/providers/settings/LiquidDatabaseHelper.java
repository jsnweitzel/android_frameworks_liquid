...
/**
 * The LiquidDatabaseHelper allows creation of a database to store Liquid specific settings for a user
 * in System, Secure, and Global tables.
 */
public class LiquidDatabaseHelper extends SQLiteOpenHelper{
    private static final String TAG = "LiquidDatabaseHelper";
    private static final boolean DEBUG = true;

    private static final String DATABASE_NAME = "liquid_settings.db";
    ...
    /**
     * Creates an instance of {@link LiquidDatabaseHelper}
     * @param context
     * @param userId
     */
    public LiquidDatabaseHelper(Context context, int userId) {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        try {
            ...
            if (DEBUG) Log.d(TAG, "Successfully created tables for liquid settings db");
        } finally {
            db.endTransaction();
        }
    }
        ...
        if (upgradeVersion < newVersion) {
            Log.w(TAG, "Got stuck trying to upgrade db. Old version: " + oldVersion
                    + ", version stuck at: " +  upgradeVersion + ", new version: "
                            + newVersion + ". Must wipe the liquid settings provider.");
